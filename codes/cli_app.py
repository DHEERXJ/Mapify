...
def get_static_latlng(lat, lng):
    ...
    url = "https://maps.googleapis.com/maps/api/staticmap?center={},{}&zoom=12&size=225x225&maptype=satellite&key=AIzaSyC54dooEEFamLsujnAc1T_kRR4tS1vZ_p8".format(
        str(lat), str(lng)
    )
    print(url)
    try:
...
def classify_lat_long(img_arr):
    ...
    pred = model.predict(np.array([cv2.resize(img_arr, (64, 64))]))
...
while True:
    ...
    img_arr = get_static_latlng(prompt[0], prompt[1])#(prompt[0], prompt[1])
